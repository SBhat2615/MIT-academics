...
struct SymbolTable *SYMTABLE[MAX_SIZE];
...
{
  char name[100];strcpy(name,t.lexeme_name);
  char type[100];strcpy(type,t.token_type);
  strcat(name,type);
...
char data_types[10][10]={"int","float","double","long","char","bool","int*","int *"};
...
int is_multiple_datatype(int from)
{
  from--;
  char *str=";";
  while(from>=0)
  {
    if(is_data_type(token_array[from].token_name))
      return from;
    if(strcmp(str,token_array[from].token_name)==0)
      return 0;
    from--;
  }
  return 0;
}
char* find_datatype_before(int from)
{
  int f=is_multiple_datatype(from);
  return token_array[f].token_name;
}
int check_variable_declaration(int from)
{
  ...
  {
    if(is_data_type(token_array[from].token_name)==1 &&
      ...
      {//printf("\nreturning from hereTWO");
    return 2;}
  }
  if((token_counter-from)>=1)
  {
    if(is_data_type(token_array[from].token_name)==1 &&
      (strcmp(str1,token_array[from+1].type)==0))
      {//printf("\nreturning from hereONE");
    return 1;}
  }
  return 0;
}
...
int table_counter=0;
void insert_to_symtable(struct lexeme t, int flag)
{
  // if(search(t.lexeme_name)==1)
  //   return;
  struct SymbolTable *cur=(struct SymbolTable*)malloc(sizeof(struct SymbolTable));
  cur->t=t;
  cur->next=NULL;
  int at;
  if(flag==1)
  ...
  if(SYMTABLE[at]==NULL)
    ...
    {
      struct SymbolTable *ele=SYMTABLE[at];
      while(ele->next!=NULL)
        ele=ele->next;
      ele->next=cur;
...
void print_symbol_table()
{
  for(int i=0;i<=table_counter;i++)
  {
    if(i==0)
      ...
      printf("\nLocal symbol table:\n");
    struct SymbolTable *ele=SYMTABLE[i];
    while(ele!=NULL)
    {
      display_lexeme(ele->t);
      printf("\n");
      ele=ele->next;
    }
  }
...
int main()
{
  ...
  for(int i=0;i<token_counter;i++)
  {
    if(check_function_declaration(i))
    {
      ...
      insert_hash(tt);
      //display_lexeme(tt);
      insert_to_symtable(tt,1);
      char *temp=")";
      i++;
      while(strcmp(temp,token_array[i].token_name)!=0)
      {
        if(check_variable_declaration(i))
        {
          lexeme tt;
          ...
          tt.return_type[0]='\0';
          insert_hash(tt);
          //printf("\nDisplaying isnide variable%d:   ",check_variable_declaration(i));
          //display_lexeme(tt);
          insert_to_symtable(tt,0);
        }
        i++;
      }
    }
    else if(check_variable_declaration(i))
    {
      ...
      char *equals="=";
      i++;
      while((strcmp(semicolon,token_array[i].token_name))!=0 && (strcmp(openbracket,token_array[i].token_name))!=0 && (strcmp(equals,token_array[i].token_name))!=0)
      {
        if(strcmp(identifier,token_array[i].type)==0)
        {
          lexeme tt;
          // char *str=strcat(token_array[i].token_name," array");
          strcpy(tt.lexeme_name,token_array[i].token_name);
          strcpy(tt.token_type,token_array[i].type);
          strcpy(tt.data_type,token_array[j].token_name);
          tt.size=give_size(token_array[j].token_name);
          tt.no_of_args=0;
