...
void declarations(){
  if((strcmp(tok.tokenname,"id")==0) || (strcmp(tok.token, "}")==0))
    return;
...
void statement_list()
{
	if (strcmp(tok.token, "}") == 0)
		return;
	else if(strcmp(tok.tokenname, "id") == 0){
	statement();
	statement_list();
	}
	else
...
void seprime()
{
	if ((strcmp(tok.token, ";") == 0)||(strcmp(tok.token, "==") == 0)||(strcmp(tok.token, "!=") == 0)||(strcmp(tok.token, "<=") == 0)||(strcmp(tok.token, ">=") == 0)||(strcmp(tok.token, ">") == 0)||(strcmp(tok.token, "<") == 0))
	    return;
...
if (strcmp(tok.token, ";") == 0||strcmp(tok.token, "==") == 0||strcmp(tok.token, "!=") == 0||strcmp(tok.token, "<=") == 0||strcmp(tok.token, ">=") == 0||strcmp(tok.token, ">") == 0||strcmp(tok.token, "<") == 0||strcmp(tok.token, "+") == 0||strcmp(tok.token, "-") == 0)
	    return;
	...
	{
	printf("Expected mulop or ; at row:%d col:%d",tok.row, tok.col);
	exit(1);
	}
